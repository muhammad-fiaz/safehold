...
fn export_and_run_with_global_merge() {
    ...
    cmd.env("SAFEHOLD_HOME", &home)
        .args(["add", "-p", "global", "-k", "SHARED", "-v", "gval"]);
    cmd.assert().success();

    let mut cmd = bin();
    cmd.env("SAFEHOLD_HOME", &home)
        .args(["add", "-p", "proj", "-k", "LOCAL", "-v", "lval"]);
    cmd.assert().success();
    ...
    cmd.assert()
        .success()
        .stdout(predicate::str::contains(".env written"));
    ...
    cmd.assert()
        .success()
        .stdout(predicate::str::contains("LOCAL=lval"));
    #[cfg(not(windows))]
