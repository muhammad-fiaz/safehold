...
#[cfg(feature = "gui")]
use crate::config::{self, env_enc_path, lock_path, Config};
#[cfg(feature = "gui")]
...
impl SafeHoldApp {
    ...
    fn ensure_loaded(&mut self, id_or_global: &str) -> Result<()> {
        ...
        match read_env_map_dir(&dir, pwd) {
            ...
            Err(e) => {
                self.add_notification(format!("Failed to load {}: {}", id_or_global, e), NotificationSeverity::Error);
                Err(e)
            }
        }
    }
    ...
    fn add_kv(&mut self, id_or_global: &str) {
        if self.new_key.trim().is_empty() {
            self.add_notification("Key cannot be empty".to_string(), NotificationSeverity::Warning);
            return;
        }
        
        let map = self.maps_cache.entry(id_or_global.to_string()).or_default();
        if map.contains_key(&self.new_key) {
            self.add_notification(format!("Key '{}' already exists. Use edit to modify.", self.new_key), NotificationSeverity::Warning);
            return;
        }
        ...
        self.add_notification(format!("Added credential '{}'", self.new_key), NotificationSeverity::Success);
        self.new_key.clear();
        self.new_val.clear();
    }
    ...
    fn delete_key(&mut self, id_or_global: &str, key: &str) {
        if let Some(map) = self.maps_cache.get_mut(id_or_global) {
            ...
            if let Err(e) = self.save_map(id_or_global) {
                ...
                self.add_notification(format!("Deleted credential '{}'", key), NotificationSeverity::Success);
    ...
    fn update_statistics(&mut self) {
        ...
        let mut all_keys: std::collections::HashMap<String, Vec<String>> = std::collections::HashMap::new();
        
        // Global project
        if let Some(map) = self.maps_cache.get("global") {
            for key in map.keys() {
                all_keys.entry(key.clone()).or_default().push("global".to_string());
            }
        }
        ...
        for project in &self.cfg.sets {
            if let Some(map) = self.maps_cache.get(&project.id) {
                for key in map.keys() {
                    all_keys.entry(key.clone()).or_default().push(project.id.clone());
                }
            }
        }
        
        self.duplicate_keys = all_keys.into_iter()
        ...
        match (lock, self.is_locked("global")) {
            (true, false) => {
                if self.new_project_password.is_empty() {
                    self.add_notification("Enter password before locking".to_string(), NotificationSeverity::Warning);
                    return;
                }
                if self.new_project_password != self.new_project_confirm_password {
                    self.add_notification("Passwords do not match".to_string(), NotificationSeverity::Warning);
                    return;
                }
                match crypto::create_lock(&self.new_project_password) {
                    Ok(li) => {
                        if let Ok(dir) = config::global_dir() {
                            ...
                            self.add_notification("Global project locked".to_string(), NotificationSeverity::Success);
                            self.new_project_password.clear();
                            self.new_project_confirm_password.clear();
                            self.refresh_config();
                        }
                    }
                    Err(e) => self.add_notification(format!("Lock failed: {}", e), NotificationSeverity::Error),
                }
            }
            (false, true) => {
                if let Ok(dir) = config::global_dir() {
                    ...
                    self.add_notification("Global project unlocked".to_string(), NotificationSeverity::Success);
                    self.refresh_config();
    ...
    fn handle_master_lock_enable(&mut self) {
        if self.master_password_input.is_empty() {
            self.add_notification("Enter master password".to_string(), NotificationSeverity::Warning);
            return;
        }
        ...
        if self.master_password_input.len() < 8 {
            self.add_notification("Master password must be at least 8 characters".to_string(), NotificationSeverity::Warning);
            return;
        }
        ...
        match crate::master_lock::enable_master_lock(&self.master_password_input) {
            Ok(_) => {
                ...
                self.add_notification("🔒 Global Master Lock ENABLED - All projects now require master password".to_string(), NotificationSeverity::Success);
            }
            Err(e) => self.add_notification(format!("Failed to enable master lock: {}", e), NotificationSeverity::Error),
        }
    }

    fn handle_master_lock_disable(&mut self) {
        if self.master_password_input.is_empty() {
            self.add_notification("Enter master password to disable".to_string(), NotificationSeverity::Warning);
            return;
        }

        match crate::master_lock::verify_master_password(&self.master_password_input) {
            Ok(true) => {
                match crate::master_lock::disable_master_lock() {
                    Ok(_) => {
                        self.master_lock_enabled = false;
                        self.show_master_lock_dialog = false;
                        self.master_password_input.clear();
                        self.master_password_confirm.clear();
                        self.add_notification("🔓 Global Master Lock DISABLED - Projects use individual settings".to_string(), NotificationSeverity::Success);
                    }
                    Err(e) => self.add_notification(format!("Failed to disable master lock: {}", e), NotificationSeverity::Error),
                }
            }
            Ok(false) => self.add_notification("Invalid master password".to_string(), NotificationSeverity::Error),
            Err(e) => self.add_notification(format!("Error verifying password: {}", e), NotificationSeverity::Error),
        }
    }
...
impl App for SafeHoldApp {
    fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
        ...
        self.notifications.retain(|n| n.timestamp.elapsed() < Duration::from_secs(5));
        ...
        egui::TopBottomPanel::top("top_nav").show(ctx, |ui| {
            ui.horizontal_wrapped(|ui| {
                ...
                // App title with icon
                ui.label(RichText::new("🔐 SafeHold").size(20.0).color(Color32::from_rgb(0, 150, 200)));
                ui.separator();
        ...
        egui::TopBottomPanel::bottom("status").show(ctx, |ui| {
            ui.horizontal(|ui| {
                ...
                if !self.duplicate_keys.is_empty() {
                    ui.label(RichText::new(format!("⚠️ {} Duplicate Keys", self.duplicate_keys.len()))
                ...
                ui.with_layout(Layout::right_to_left(Align::Center), |ui| {
                    if let Some(selected) = &self.selected {
                        ...
                        let lock_status = if self.is_locked(selected) { " 🔒" } else { " 🔓" };
...
impl SafeHoldApp {
            ...
            ui.group(|ui| {
                ...
                if self.is_locked("global") {
                    ui.horizontal(|ui| {
                        ...
                        if ui.add(egui::TextEdit::singleline(pw).password(true)).changed() {
                            self.maps_cache.remove("global");
                        }
            ...
            egui::ScrollArea::vertical().show(ui, |ui| {
                if self.cfg.sets.is_empty() {
                    ui.centered_and_justified(|ui| {
                        ui.label(RichText::new("No projects yet. Create your first project!")
                            .size(16.0)
                            .color(Color32::GRAY));
                    });
                } else {
                    for project in &self.cfg.sets.clone() {
                        if !self.search_filter.is_empty() && 
                           !project.name.to_lowercase().contains(&self.search_filter.to_lowercase()) {
                            continue;
                        }
                        ...
                        ui.group(|ui| {
                            ...
                            ui.horizontal(|ui| {
                                ...
                                ui.label(RichText::new(format!("{} {}", lock_icon, project.name))
                                    .size(16.0));
    ...
    fn render_credentials_tab(&mut self, ui: &mut Ui) {
        if let Some(selected) = &self.selected.clone() {
            ...
            ui.centered_and_justified(|ui| {
                ui.label(RichText::new("Select a project from the Projects tab to view its credentials")
                    .size(16.0)
                    .color(Color32::GRAY));
            });
        }
        ...
        ui.vertical(|ui| {
            ...
            ui.group(|ui| {
                ...
                ui.horizontal(|ui| {
                    ...
                    let can_add = !self.global_new_key.is_empty() && !self.global_new_value.is_empty();
                    if ui.add_enabled(can_add, egui::Button::new("➕ Add")).clicked() {
                        ...
                        if let Ok(global_dir) = config::global_dir() {
                            ...
                            if let Err(e) = self.write_env_map(&global_dir, &map) {
                                self.add_notification(format!("Failed to add credential: {}", e), NotificationSeverity::Error);
                            } else {
                                self.add_notification(format!("Added global credential '{}'", self.global_new_key), NotificationSeverity::Success);
                                self.global_credentials = map;
            ...
            if self.global_credentials.is_empty() {
                ui.vertical_centered(|ui| {
                    ui.label(RichText::new("No global credentials found")
                        .size(16.0)
                        .color(Color32::GRAY));
                });
            } else {
                            ...
                            ui.horizontal(|ui| {
                                ...
                                ui.with_layout(Layout::right_to_left(Align::Center), |ui| {
                                    ...
                                    if ui.button("📋 Copy").clicked() {
                                        ui.ctx().copy_text(value.clone());
                                        self.add_notification("Value copied to clipboard".to_string(), NotificationSeverity::Info);
                                    }
                                });
                            });
                        });
                ...
                ui.separator();
                ui.label(format!("📊 Total: {} global credential(s)", self.global_credentials.len()));
            }
        });
    }
    ...
    fn render_statistics_tab(&mut self, ui: &mut Ui) {
        ui.vertical(|ui| {
            ...
            ui.group(|ui| {
                ...
                ui.horizontal(|ui| {
                    ui.vertical(|ui| {
                        ui.label(RichText::new(format!("{}", self.total_projects)).size(24.0).color(Color32::from_rgb(0, 150, 200)));
                        ui.label("Total Projects");
                    });
                    ui.separator();
                    ui.vertical(|ui| {
                        ui.label(RichText::new(format!("{}", self.total_credentials)).size(24.0).color(Color32::from_rgb(0, 200, 100)));
                        ui.label("Total Credentials");
                    });
                    ui.separator();
                    ui.vertical(|ui| {
                        ui.label(RichText::new(format!("{}", self.duplicate_keys.len())).size(24.0).color(Color32::from_rgb(200, 150, 0)));
            ...
            if !self.duplicate_keys.is_empty() {
                ui.group(|ui| {
                    ui.label(RichText::new("Duplicate Keys Analysis").strong().color(Color32::from_rgb(200, 150, 0)));
                    ...
                    ui.horizontal(|ui| {
                        ...
                        if ui.add_enabled(can_create, egui::Button::new("✅ Create")).clicked() {
                            ...
                            match store::cmd_create(args) {
                                Ok(()) => {
                                    self.add_notification("Project created successfully".to_string(), NotificationSeverity::Success);
                                ...
                                Err(e) => {
                                    self.add_notification(format!("Create failed: {}", e), NotificationSeverity::Error);
                                }
                            }
                        }
    ...
    fn render_add_credential_dialog(&mut self, ctx: &egui::Context) {
        if !self.show_add_credential { return; }
        ...
        egui::Window::new("Add Credential")
            ...
            .show(ctx, |ui| {
                ui.vertical(|ui| {
                    ...
                    ui.horizontal(|ui| {
                        let can_add = !self.new_key.trim().is_empty() && !self.new_val.trim().is_empty();
    ...
    fn render_export_dialog(&mut self, ctx: &egui::Context) {
        ...
        egui::Window::new("Export Project")
            ...
            .show(ctx, |ui| {
                ui.vertical(|ui| {
                    ui.label(format!("Export credentials from project: {}", self.export_project));
                    ui.separator();
                    ...
                    ui.horizontal(|ui| {
                        ...
                        egui::ComboBox::from_label("")
                            ...
                            .show_ui(ui, |ui| {
                                ui.selectable_value(&mut self.export_format, "env".to_string(), "Environment Variables (.env)");
                                ui.selectable_value(&mut self.export_format, "json".to_string(), "JSON");
                                ui.selectable_value(&mut self.export_format, "yaml".to_string(), "YAML");
                            });
                    });
                    ...
                    ui.horizontal(|ui| {
                        if ui.button("📤 Export").clicked() {
                            match envops::cmd_export(ExportArgs {
                                ...
                                global: false,
                                file: if self.export_file.is_empty() { None } else { Some(self.export_file.clone()) },
                                force: false,
                                temp: false,
                            }) {
                                Ok(()) => {
                                    self.add_notification("Export completed successfully".to_string(), NotificationSeverity::Success);
                                    self.show_export = false;
                                }
                                Err(e) => {
                                    self.add_notification(format!("Export failed: {}", e), NotificationSeverity::Error);
                                }
                            }
                        }
    ...
    fn render_run_command_dialog(&mut self, ctx: &egui::Context) {
        if !self.show_run_command { return; }
        ...
        egui::Window::new("Run Command")
            ...
            .show(ctx, |ui| {
                ui.vertical(|ui| {
                    ui.label(format!("Run command with environment from project: {}", self.run_project));
                    ui.separator();
                    ...
                    ui.horizontal(|ui| {
                        if ui.button("▶️ Run").clicked() {
                            match envops::cmd_run(RunArgs {
                                ...
                                command: self.run_command.split_whitespace().map(|s| s.to_string()).collect(),
                                with_global: false,
                            }) {
                                Ok(()) => {
                                    self.add_notification("Command executed".to_string(), NotificationSeverity::Success);
                                    self.show_run_command = false;
                                }
                                Err(e) => {
                                    self.add_notification(format!("Command failed: {}", e), NotificationSeverity::Error);
                                }
                            }
                        }
                                ...
                                if let Some(selected) = &self.selected.clone() {
                                    if !self.update_key.is_empty() && !self.update_value.is_empty() {
                                        ...
                                        if let Ok(mut map) = read_env_map_dir(&dir, password.as_deref()) {
                                            map.insert(self.update_key.clone(), self.update_value.clone());
                                            if let Err(e) = write_env_map_dir(&dir, &map, password.as_deref()) {
                                                self.add_notification(format!("Failed to update credential: {}", e), NotificationSeverity::Error);
                                            } else {
                                                self.add_notification(format!("Updated credential '{}'", self.update_key), NotificationSeverity::Success);
                            ...
                            if ui.add_enabled(can_add, egui::Button::new("✅ Add")).clicked() {
                                if let Ok(global_dir) = config::global_dir() {
                                    if let Ok(mut map) = read_env_map_dir(&global_dir, None) {
                                        map.insert(self.global_new_key.clone(), self.global_new_value.clone());
                                        if let Err(e) = write_env_map_dir(&global_dir, &map, None) {
                                            self.add_notification(format!("Failed to add global credential: {}", e), NotificationSeverity::Error);
                                        } else {
                                            self.add_notification(format!("Added global credential '{}'", self.global_new_key), NotificationSeverity::Success);
                                            self.global_credentials = map;
                        ...
                        ui.horizontal(|ui| {
                            if ui.button("✅ Update").clicked() {
                                if !self.global_update_value.is_empty() {
                                    if let Ok(global_dir) = config::global_dir() {
                                        if let Ok(mut map) = read_env_map_dir(&global_dir, None) {
                                            map.insert(self.global_update_key.clone(), self.global_update_value.clone());
                                            if let Err(e) = write_env_map_dir(&global_dir, &map, None) {
                                                self.add_notification(format!("Failed to update global credential: {}", e), NotificationSeverity::Error);
                                            } else {
                                                self.add_notification(format!("Updated global credential '{}'", self.global_update_key), NotificationSeverity::Success);
                                                self.global_credentials = map;
    ...
    fn render_maintenance_dialogs(&mut self, ctx: &egui::Context) {
            ...
            egui::Window::new("Clean Cache Confirmation")
                ...
                .show(ctx, |ui| {
                    ...
                    ui.group(|ui| {
                        ...
                        ui.separator();
                        ui.label(RichText::new("⚠️ Your credentials will NOT be affected").strong().color(Color32::from_rgb(0, 150, 0)));
                    });
                    
                    ui.horizontal(|ui| {
                        if ui.button(RichText::new("🗑️ Clean Cache").color(Color32::from_rgb(255, 140, 0))).clicked() {
                            match envops::cmd_clean_cache(true) {
                                Ok(()) => self.add_notification("✅ Cache cleaned successfully".to_string(), NotificationSeverity::Success),
                                Err(e) => self.add_notification(format!("❌ Failed to clean cache: {}", e), NotificationSeverity::Error),
                            }
        ...
        if self.show_delete_all_confirm {
            egui::Window::new("⚠️ DANGER: Delete All Data")
                ...
                .show(ctx, |ui| {
                    ui.vertical_centered(|ui| {
                        ui.label(RichText::new("🚨 PERMANENT DATA DELETION").heading().color(Color32::RED));
                        ui.separator();
                    });
                    
                    ui.group(|ui| {
                        ui.label(RichText::new("THIS WILL PERMANENTLY DELETE:").strong().color(Color32::RED));
                        ui.label(RichText::new("• All credential projects and their encrypted data").color(Color32::RED));
                        ...
                        ui.separator();
                        ui.label(RichText::new("⚠️ THIS ACTION CANNOT BE UNDONE!").strong().color(Color32::RED));
                        ui.label(RichText::new("⚠️ ALL YOUR CREDENTIALS WILL BE LOST!").strong().color(Color32::RED));
                    });
                    ...
                    ui.horizontal(|ui| {
                        if ui.button(RichText::new("💥 DELETE ALL DATA").color(Color32::RED)).clicked() {
                            match envops::cmd_delete_all(true) {
                                Ok(()) => {
                                    self.add_notification("💥 All data has been permanently deleted".to_string(), NotificationSeverity::Success);
                                ...
                                Err(e) => self.add_notification(format!("❌ Failed to delete data: {}", e), NotificationSeverity::Error),
                            }
                    ...
                    match self.master_lock_action {
                        Some(true) => {
                            ...
                            ui.group(|ui| {
                                ...
                                ui.separator();
                                ui.label(RichText::new("⚠️ This will change how ALL your projects are accessed").color(Color32::from_rgb(255, 140, 0)));
                            });
                            
                            ui.group(|ui| {
                                ui.label(RichText::new("Create Master Password").strong());
                                ui.label("Password must be at least 8 characters:");
                                ui.add(egui::TextEdit::singleline(&mut self.master_password_input)
                                    .password(true)
                                    .hint_text("Enter master password"));
                                ui.add(egui::TextEdit::singleline(&mut self.master_password_confirm)
                                    .password(true)
                            ...
                            ui.horizontal(|ui| {
                                ...
                                if ui.add_enabled(passwords_match, egui::Button::new("🔒 Enable Master Lock")).clicked() {
                        ...
                        Some(false) => {
                            ...
                            ui.horizontal(|ui| {
                                ...
                                if ui.add_enabled(can_disable, egui::Button::new("🔓 Disable Master Lock")).clicked() {
