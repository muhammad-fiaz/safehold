...
pub fn load_master_lock_info() -> Result<MasterLockInfo> {
    ...
    Ok(MasterLockInfo {
        enabled: json.get("enabled").and_then(|v| v.as_bool()).unwrap_or(false),
        password_hash: json.get("password_hash").and_then(|v| v.as_str()).map(|s| s.to_string()),
    })
}
...
pub fn verify_master_password(password: &str) -> Result<bool> {
    ...
    let stored_hash = info.password_hash
